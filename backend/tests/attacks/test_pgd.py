"""Unit tests for the PGD attack module (ART-based)."""
...
class TestPGD:
    ...
    def _run(self, model: nn.Module, data: tuple[np.ndarray, np.ndarray]) -> dict:
        attack = PGD(epsilon=self.EPSILON, max_iter=10)  # fewer iters for speed
    ...
    def test_pgd_stronger_than_fgsm(
        ...
        """PGD (iterative) should degrade accuracy at least as much as FGSM."""
        from backend.attacks.fgsm import FGSM

        fgsm_result = FGSM(epsilon=self.EPSILON).run(trained_model, mnist_batch)
        pgd_result = self._run(trained_model, mnist_batch)
        assert pgd_result["adv_accuracy"] <= fgsm_result["adv_accuracy"]
